...
function absSrcPath(rel) {
  console.log('raw dirname:', ('' + new URL('.', import.meta.url)));
  let dirname = ('' + new URL('.', import.meta.url)).replace(/file:\/\/(?:\/([A-Z]:\/))?/, '$1');
  console.log('raw2 dirname:', dirname);
	let p = path.join(/* __dirname */ dirname, '..', rel);
	return path.resolve(p);
}

console.log("__dirname/../x = ", absSrcPath('x'));
